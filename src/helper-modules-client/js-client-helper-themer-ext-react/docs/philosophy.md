...
Font registration and component construction are app concerns. That logic stays in the app, passed through the `transform` seam. The module owns the plumbing; the app owns the transform.
...
- **Component building**: build a themed component system from the emitted tokens
...
| Themed component system | The app, via `transform` | The module knows no components |
