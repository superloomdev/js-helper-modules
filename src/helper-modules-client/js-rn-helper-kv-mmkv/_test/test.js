...
describe('KvMmkv loader', function () {
  ...
  it('should construct the MMKV instance with id from config', function () {
    ...
    // Verify the stub received the id by writing and reading a key
    fresh.writeRecordSync('key', 'val');
    assert.strictEqual(fresh.getRecordSync('key').value, 'val');
  });

  it('should pass encryptionKey to the MMKV constructor when configured', function () {
    let capturedOptions = null;

    function CapturingMmkv (options) {
      capturedOptions = options;
      this._data = {};
    }

    CapturingMmkv.prototype.getString = function (k) { return this._data[k]; };
    CapturingMmkv.prototype.set = function (k, v) { this._data[k] = String(v); };
    CapturingMmkv.prototype.delete = function (k) { delete this._data[k]; };
    CapturingMmkv.prototype.contains = function (k) { return Object.prototype.hasOwnProperty.call(this._data, k); };
    ...
    require('helper-kv-mmkv')({
      ...
      NAMESPACE: 'enc',
      INSTANCE_ID: 'enc-instance',
      ENCRYPTION_KEY: 'my-secret-key'
    });

    assert.strictEqual(capturedOptions.id, 'enc-instance');
    assert.strictEqual(capturedOptions.encryptionKey, 'my-secret-key');
  });

  it('should omit encryptionKey when not configured', function () {
    let capturedOptions = null;
    ...
    require('helper-kv-mmkv')({
      ...
      NAMESPACE: 'noenc',
      INSTANCE_ID: 'noenc-instance'
    });

    assert.strictEqual(capturedOptions.id, 'noenc-instance');
    assert.strictEqual(capturedOptions.encryptionKey, undefined);
  });
...
describe('getRecordSync', function () {
  ...
  it('should return DESERIALIZE_FAILED when stored value is not valid JSON', function () {
    ...
    fresh.MmkvStub._rawSet('corrupt:bad', 'not-json{');
...
describe('writeRecordSync', function () {
  ...
  it('should return INVALID_VALUE when value is undefined', function () {
    ...
    assert.strictEqual(result.success, false);
    assert.strictEqual(result.error.type, 'helper-kv-mmkv/invalid-value');
  });
...
describe('batchGetRecordsSync', function () {
  ...
  it('should return INVALID_KEYS when argument is not an array', function () {
    ...
    assert.strictEqual(result.success, false);
    assert.strictEqual(result.error.type, 'helper-kv-mmkv/invalid-keys');
  });
...
describe('namespace isolation', function () {
  ...
  it('should not leak keys between namespaces when two instances share one MMKV instance', function () {
