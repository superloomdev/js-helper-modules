...
// cache_code, giving O(1) getCache/setCache/deleteCache and
// partition-scoped deleteCacheByPrefix/clearCache/listCacheCodes.
//
...
// runs).
//
// deleteCacheByPrefix, clearCache, and listCacheCodes use Query with
// begins_with on the sort key, scoped to one partition key (namespace).
// This is O(N) over the partition, not the entire table - a significant
// advantage over the flat-keyspace Valkey adapter.
//
...
// JSON.parse on getCache. The cache module passes raw JavaScript
// objects; the adapter serializes before handing to Lib.DynamoDB and
// deserializes before returning to the cache module. DynamoDB's
// Document Client stores the JSON string as a single string attribute
// (VALUE_FIELD).
//
...
// entry items. setCacheLock uses Lib.DynamoDB.writeRecordIfNotExists
// (atomic PutItem with attribute_not_exists condition) with an expiry
// timestamp.
  ...
  const Store = {
    ...
    *********************************************************************/
    getCache: async function (instance, namespace, cache_code) {
      ...
      // (cache reads must see the most recent write, especially in getOrFetchCache
      ...
      try {
        ...
        Lib.Debug.debug('[helper-cache-store-dynamodb] getCache deserialization failed', {
          namespace: namespace,
    ...
    *********************************************************************/
    setCache: async function (instance, namespace, cache_code, value, ttl_seconds) {
      ...
      try {
        ...
        Lib.Debug.debug('[helper-cache-store-dynamodb] setCache serialization failed', {
          namespace: namespace,
          cache_code: cache_code,
          error: err && err.message
        });
    ...
    *********************************************************************/
    ...
    /********************************************************************
    Selective mass invalidation. Query for all items in the namespace
    matching the cache_code_prefix, then batch-delete them.
    Short-circuits on zero matches. O(N) over the partition, not the
    entire table. The cache_code_prefix is required. To wipe every
    entry in a namespace, use clearCache instead.
    ...
    *********************************************************************/
    deleteCacheByPrefix: async function (instance, namespace, cache_code_prefix) {
      ...
      // Build the query parameters - prefix is required, use begins_with on the sort key
      const query_params = {
        pk: namespace,
        pkName: CONFIG.PARTITION_KEY,
        skCondition: 'begins_with(' + CONFIG.SORT_KEY + ', :sk)',
        skValues: { ':sk': cache_code_prefix }
      };
      ...
      if (query_result.success === false) {
        _Store.logDriverFailure('deleteCacheByPrefix (query)', query_result.error);
        return {
          success: false,
          deleted_count: 0,
          error: ERRORS.SERVICE_UNAVAILABLE
      ...
      // Filter out lock items - deleteCacheByPrefix operates on cache entries only,
      ...
      if (delete_result.success === false) {
        _Store.logDriverFailure('deleteCacheByPrefix (batchDelete)', delete_result.error);
        return {
          success: false,
          deleted_count: 0,
          error: ERRORS.SERVICE_UNAVAILABLE
    ...
    /********************************************************************
    Wipe every cache entry in the namespace. Query for all items in
    the namespace (no sort-key condition), then batch-delete them.
    Short-circuits on zero matches. O(N) over the partition, not the
    entire table.
    ...
    *********************************************************************/
    clearCache: async function (instance, namespace) {
      ...
      // Build the query parameters - no prefix, query by partition key only
      ...
      // Query for all items in the namespace
      const query_result = await Lib.DynamoDB.query(
        instance,
        CONFIG.TABLE_NAME,
        query_params
      ...
      // Return a service error if the query failed
      if (query_result.success === false) {
        _Store.logDriverFailure('clearCache (query)', query_result.error);
        return {
          success: false,
          deleted_count: 0,
          error: ERRORS.SERVICE_UNAVAILABLE
      ...
      // Filter out lock items - clearCache operates on cache entries only,
      // not on distributed lock items which share the same partition
      const cache_items = query_result.items.filter(function (item) {
        return item[CONFIG.SORT_KEY].indexOf(CONFIG.LOCK_SORT_KEY_PREFIX) !== 0;
      });
      ...
      // Return a service error if the batch delete failed
      if (delete_result.success === false) {
        _Store.logDriverFailure('clearCache (batchDelete)', delete_result.error);
        return {
          success: false,
          deleted_count: 0,
          error: ERRORS.SERVICE_UNAVAILABLE
    ...
    *********************************************************************/
    listCacheCodes: async function (instance, namespace, cache_code_prefix) {
      ...
      // Return a service error if the query failed
      if (query_result.success === false) {
        _Store.logDriverFailure('listCacheCodes (query)', query_result.error);
        return {
          success: false,
          cache_codes: [],
          error: ERRORS.SERVICE_UNAVAILABLE
    ...
    *********************************************************************/
    getCacheExists: async function (instance, namespace, cache_code) {
      ...
      // (same rationale as getCache: must see the most recent write)
      const result = await Lib.DynamoDB.getRecord(
        instance,
        CONFIG.TABLE_NAME,
        _Store.composeKey(namespace, cache_code),
