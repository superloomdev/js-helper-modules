...
//   - refresh_token_hash and refresh_family_id round-trip
//   - updateSessionActivity edge cases (empty updates, last_active_at write)
//   - deleteSessions bulk path (single, multiple, empty)
//   - getSession not-found path
//   - cleanupExpiredSessions on empty table
//   - large multi-actor list isolation
//
...
describe('Tier 1: _Store identifier quoting', function () {
  ...
  it('Q wraps a clean name in backticks', function () {
  ...
  it('Q rejects a table_name containing a backtick', function () {
...
describe('Tier 1: value coercions', { concurrency: false }, function () {
  ...
  it('round-trips client_is_browser = true as TINYINT(1) and reads back as true', async function () {
  ...
  it('round-trips client_is_browser = false as TINYINT(1) and reads back as false', async function () {
