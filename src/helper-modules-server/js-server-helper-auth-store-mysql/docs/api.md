...
This adapter is loaded by the Auth parent module via the factory protocol. The parent calls these methods to satisfy its persistence requirements. This page documents the contract as implemented for MySQL, with backend-specific semantics noted where they matter.
...
The driver's underlying error is logged via `Lib.Debug.debug` with the SQL statement and driver-side error type. It is not surfaced to the caller. The Auth parent translates `ERRORS.SERVICE_UNAVAILABLE` into the appropriate response.

### `setupNewStore(instance)`

Idempotent table-and-index setup. Issues one `CREATE TABLE IF NOT EXISTS` statement with the `expires_at` index inlined as `INDEX idx_expires_at (expires_at)`. MySQL does not support `CREATE INDEX IF NOT EXISTS` as a standalone statement, so the inline form is the only single-statement idempotent path.
...
Reads a single session row by its composite primary key, then verifies the secret hash before returning the record.
...
Full UPSERT. Inserts the row, or replaces every mutable column when the composite primary key collides.
...
The UPSERT uses `INSERT ... ON DUPLICATE KEY UPDATE col = VALUES(col)`. This is MySQL's native syntax; differs from Postgres's `INSERT ... ON CONFLICT (pk) DO UPDATE SET col = EXCLUDED.col`. The `VALUES(col)` function references the values that would have been inserted, and the `DO UPDATE` list excludes identity and per-install columns so re-inserts cannot tamper with the original creation metadata. The full immutability rules are documented in [schema.md](schema.md).
...
MySQL has no native row-level TTL, so this is the only path for expired-row deletion. See [cleanup.md](cleanup.md) for the recommended scheduling mechanism and the MySQL Event Scheduler alternative.
