// Tests for helper-cache-store-mongodb
// Integration tests against a real MongoDB instance via Docker Compose.
...
// Native MongoDB client for setup/teardown (drop collection, create TTL index)
const adminClient = new MongoClient(Config.mongodb_connection_string);
...
before(async function () {
  ...
  // Drop the test collection if it exists, then recreate it
  const db = adminClient.db(Config.mongodb_database);
  ...
  // Create a TTL index on the expiry field so expired documents are
  // automatically deleted by MongoDB's background sweeper
  await db.collection(TEST_COLLECTION).createIndex(
    { expires_at: 1 },
...
beforeEach(async function () {
  ...
  // Clear all documents in the test collection before each test
  const db = adminClient.db(Config.mongodb_database);
...
describe('Store tier: setCache + getCache round-trip', function () {
  ...
  it('round-trips an object value through the store (native BSON, no serialization)', async function () {
...
describe('listCacheCodes without cache_code_prefix', function () {
  ...
  it('omits entries whose TTL has passed', async function () {
    ...
    // Advance past the short TTL only - the MongoDB sweeper has not run
...
describe('Cache tier: getOrFetchCache - locked (stampede protection)', function () {
  ...
  it('concurrent callers - fetcher called exactly once', async function () {
    ...
    assert.strictEqual(callCount, 1, 'fetcher should be called exactly once with MongoDB lock');
  });
...
// ============================================================================
// 17. NATIVE BSON STORAGE (store tier)
// ============================================================================

describe('Store tier: native BSON storage', function () {

  it('stores and retrieves nested objects without serialization', async function () {
    const instance = createInstance();
  ...
  it('stores and retrieves a value with no TTL (persists until deleted)', async function () {
    const instance = createInstance();

    await store.setCache(instance, 'NS', 'no-ttl', 'persistent-value');
    const result = await store.getCache(instance, 'NS', 'no-ttl');

    assert.strictEqual(result.success, true);
    assert.strictEqual(result.value, 'persistent-value');
  });
