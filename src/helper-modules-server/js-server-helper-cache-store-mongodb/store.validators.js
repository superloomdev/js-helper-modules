// Info: Config validator for helper-cache-store-mongodb.
  ...
  *********************************************************************/
  validateConfig: function (config) {
    ...
    if (
      ...
      throw new Error('[helper-cache-store-mongodb] config.COLLECTION_NAME is required (non-empty string)');
    }
    ...
    if (
      ...
      throw new Error('[helper-cache-store-mongodb] config.VALUE_FIELD is required (non-empty string)');
    }
    ...
    if (
      ...
      throw new Error('[helper-cache-store-mongodb] config.EXPIRY_FIELD is required (non-empty string)');
    }

    // LOCK_ID_PREFIX is required and must be a non-empty string
    if (
      Lib.Utils.isNullOrUndefined(config.LOCK_ID_PREFIX) ||
      !Lib.Utils.isString(config.LOCK_ID_PREFIX) ||
      config.LOCK_ID_PREFIX === ''
    ) {
      throw new Error('[helper-cache-store-mongodb] config.LOCK_ID_PREFIX is required (non-empty string)');
    }
