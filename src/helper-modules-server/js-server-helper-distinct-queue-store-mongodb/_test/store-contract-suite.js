  ...
  return async function runContractSuite () {
    ...
    const createRecord = function (overrides) {
      return Object.assign({
        ...
        sort_key: 'resource_1#' + Date.now() + '#' + Lib.Crypto.generateCompactUUID(),
        payload: { data: 'test' },
        action: 'test_action',
        toc: Date.now()
      }, overrides);
    ...
    await describe('writeRecord', async function () {
      ...
      await it('should write multiple records for same resource', async function () {
        ...
        const record1 = createRecord({ resource_id, data_version: 1000, sort_key: resource_id + '#1000#aaaa0000' });
        const record2 = createRecord({ resource_id, data_version: 2000, sort_key: resource_id + '#2000#bbbb1111' });
    ...
    await describe('queryByResourceId', async function () {
      ...
      await it('should return records sorted by data_version ascending', async function () {
        ...
        const record3 = createRecord({ tenant_id, resource_id, data_version: 3000, sort_key: resource_id + '#3000#cccc2222' });
        const record1 = createRecord({ tenant_id, resource_id, data_version: 1000, sort_key: resource_id + '#1000#aaaa0000' });
        const record2 = createRecord({ tenant_id, resource_id, data_version: 2000, sort_key: resource_id + '#2000#bbbb1111' });
    ...
    await describe('deleteByDataVersionLte', async function () {
      ...
      await it('should delete records at or below the boundary', async function () {
        ...
        await store.writeRecord(instance, createRecord({ tenant_id, resource_id, data_version: 1000, sort_key: resource_id + '#1000#aaaa0000' }));
        await store.writeRecord(instance, createRecord({ tenant_id, resource_id, data_version: 2000, sort_key: resource_id + '#2000#bbbb1111' }));
        await store.writeRecord(instance, createRecord({ tenant_id, resource_id, data_version: 3000, sort_key: resource_id + '#3000#cccc2222' }));
      ...
      await it('should preserve records above the boundary', async function () {
        ...
        await store.writeRecord(instance, createRecord({ tenant_id, resource_id, data_version: 5000, sort_key: resource_id + '#5000#aaaa0000' }));
      ...
      await it('should only delete records for the specified tenant and resource', async function () {
        ...
        await store.writeRecord(instance, createRecord({ tenant_id: 'tenant_a', resource_id: 'res_1', data_version: 1000, sort_key: 'res_1#1000#aaaa0000' }));
        await store.writeRecord(instance, createRecord({ tenant_id: 'tenant_a', resource_id: 'res_2', data_version: 1000, sort_key: 'res_2#1000#bbbb1111' }));
        await store.writeRecord(instance, createRecord({ tenant_id: 'tenant_b', resource_id: 'res_1', data_version: 1000, sort_key: 'res_1#1000#cccc2222' }));
