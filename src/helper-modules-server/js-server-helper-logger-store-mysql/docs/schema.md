...
- **`idx_<table>_entity_sort`** - compound `(tenant_id, entity_type, entity_id, sort_key)` covering `getLogsByEntity` + cursor pagination (`sort_key < ?` then `ORDER BY sort_key DESC`).
- **`idx_<table>_actor_sort`** - compound `(tenant_id, actor_type, actor_id, sort_key)` covering `getLogsByActor` + cursor pagination.
