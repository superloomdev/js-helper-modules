...
|---|---|---|
| `shared_libs` | Object | Dependency container (`Lib`). Must have `Lib.Utils`, `Lib.Debug`, `Lib.MongoDB` |
| `config` | Object | `{ collection_name }`. See [configuration.md](configuration.md) |
...
The adapter throws an `Error` if `config.collection_name` is missing, null, or empty.
