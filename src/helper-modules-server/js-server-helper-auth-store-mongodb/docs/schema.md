...
**Why `\u001F` as the separator.** ASCII Unit Separator is a non-printable control character that cannot appear in any human-readable identifier. The Auth parent's input validation rejects `actor_id` and `tenant_id` values containing `\u001F` as belt-and-braces, but the constraint is vacuous in practice because the character is not typeable through normal input methods. This follows the precedent set by `helper-distinct-queue-store-dynamodb`, which uses `\u001F` as its sort key delimiter for the same reason.
