...
import loggerStoreSqliteLoader from 'helper-logger-store-sqlite';
import loggerLoader from 'helper-logger';
import runSharedStoreSuite from './store-contract-suite.js';

const { Lib } = loader();
const LoggerStoreSQLiteFactory = loggerStoreSqliteLoader;
...
describe('Tier 1: addLog and getLogsByEntity round-trip', { concurrency: false }, function () {
  ...
  it('getLogsByEntity supports cursor pagination', async function () {
    ...
    const page1 = await store.getLogsByEntity(instance, {
      tenant_id: '', entity_type: 'user', entity_id: 'rt-user-4', limit: 2
    });
    ...
    const page2 = await store.getLogsByEntity(instance, {
      tenant_id: '', entity_type: 'user', entity_id: 'rt-user-4', limit: 2, cursor: page1.next_cursor
    });
    ...
    const page3 = await store.getLogsByEntity(instance, {
      tenant_id: '', entity_type: 'user', entity_id: 'rt-user-4', limit: 2, cursor: page2.next_cursor
    });
...
describe('Tier 1: cleanupExpiredLogs', { concurrency: false }, function () {
  ...
  it('deletes only expired rows and returns correct count', async function () {
    ...
    const remaining = await store.getLogsByEntity(instance, {
      tenant_id: '', entity_type: 'user', entity_id: 'u-persist'
    });
