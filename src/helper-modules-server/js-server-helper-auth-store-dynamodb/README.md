...
An AWS DynamoDB-backed implementation of the [helper-auth](https://github.com/superloomdev/superloom/tree/main/src/helper-modules-server/js-server-helper-auth) module's storage contract. Call it with a `Lib` container and config to get a ready-to-use store object, then pass that as `Store` to the Auth parent. Table provisioning is out-of-band (IaC). Part of [Superloom](https://superloom.dev).
...
- [`helper-auth-store-mysql`](https://github.com/superloomdev/superloom/tree/main/src/helper-modules-server/js-server-helper-auth-store-mysql) - MySQL / MariaDB
...
It receives Utils, Debug, and DynamoDB through the `Lib` container injected by the caller. For the full dependency breakdown, see [`docs/configuration.md`](docs/configuration.md).
