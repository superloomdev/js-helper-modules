...
- **options.namespace**: String, required. Logical owner namespace.
...
- **options.namespace**, **options.key**, **options.value** (non-empty string), **options.max_fail_count** (integer `> 0`): all required.
...
| `VERIFY_NOT_FOUND` | `verify` with no record for `(namespace, key)` | `verify` |
...
2. `getRecord(namespace, key)`. Store failure becomes `VERIFY_SERVICE_UNAVAILABLE`.
...
- **One `(namespace, key)` pair holds at most one active record.** A new `create*` call replaces the previous record. There is no accumulation.
