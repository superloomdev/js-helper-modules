...
**Fail counter.** Each failed `verify` call increments `fail_count` in-place. Once `fail_count >= max_fail_count`, the record is locked out (`VERIFY_MAX_FAILS`) until a new code is created. The counter resets to `0` on every successful create.
...
Whether `cleanupExpiredRecords` needs to be scheduled, and at what cadence, depends on the chosen storage adapter (some adapters' native TTL makes the cron optional). See each adapter package's README for the per-backend recommendation. For the runtime-shape difference between scheduling cleanup in a persistent server and a serverless function, see [Runtime](runtime.md).
...
Once verified, the record is deleted immediately. A verified code cannot be reused because it no longer exists. This is a deliberate security property: verification is strictly one-time. See [Runtime](runtime.md) for the serverless caveat where the background delete may be deferred.
