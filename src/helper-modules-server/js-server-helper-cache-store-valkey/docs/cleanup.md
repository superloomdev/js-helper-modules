...
Valkey handles expiry natively. When `setCache` is called with a `ttl_seconds` value, the key is stored with `SET key value EX ttl_seconds`. Valkey deletes the key automatically when the TTL expires - no application-side scheduling is required.
