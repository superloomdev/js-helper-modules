...
- [The `sort_key` Field](#the-sort_key-field)
- [Data Payload Convention](#data-payload-convention)
- [Retention Quick Reference](#retention-quick-reference)
...
**Entity.** The *subject* of the action: the thing that was changed, created, deleted, or read. An entity has a `type` (what kind of thing it is) and an `id` (which specific instance).
...
**Actor.** The *agent* who triggered the action: the person, system, or automated process that caused the event. An actor has a `type` and an `id`.
...
### Why is IP encryption optional?

Some environments need IPs for fraud detection. Others must encrypt them for privacy compliance. The module supports both. Set or omit `IP_ENCRYPT_KEY`.
