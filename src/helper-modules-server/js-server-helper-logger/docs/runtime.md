...
**Persistent server.** The Node process keeps running between requests. A backgrounded write that has not completed before the response is sent continues on the same event loop. No special care is required; this is the normal case.
...
Whether `cleanupExpiredLogs(instance)` needs to be scheduled depends on the chosen storage adapter (some adapters' native TTL makes the cron optional). See the chosen adapter's README for whether scheduling is required and the recommended cadence.
