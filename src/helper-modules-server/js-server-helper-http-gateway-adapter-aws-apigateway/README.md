...
| Method | Description |
|--------|-------------|
| `extractRequest(event, context, callback)` | Returns normalized request fields and a `response_handler` closure |
| `buildResponseEnvelope(status, headers, body)` | Builds the `{ statusCode, headers, body, isBase64Encoded }` Lambda response envelope |
| `getCountryCode(headers)` | Returns `CloudFront-Viewer-Country` header value or `null` |
...
When a CloudFront distribution sits in front of API Gateway and forwards the `CloudFront-Viewer-Country` header, `getCountryCode` returns the ISO 3166-1 alpha-2 country code string. Returns `null` when absent.
