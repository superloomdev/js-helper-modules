...
For setting cookies in the response, use `Gateway.buildCookie(...)` and pass the descriptor to `Gateway.returnHttpResponse(...)`. The gateway serializes each cookie and collects them into a `Set-Cookie` header array in the response envelope.
