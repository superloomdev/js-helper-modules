// Info: Config validator for js-server-helper-verify-store-dynamodb.
  ...
  *********************************************************************/
  validateConfig: function (Lib, config) {
    ...
    if (
      ...
      throw new Error('[js-server-helper-verify-store-dynamodb] config must be an object');
    }
    ...
    if (
      ...
      throw new Error('[js-server-helper-verify-store-dynamodb] config.table_name is required');
    }

    // lib_dynamodb is required - the caller must inject the DynamoDB helper
    if (Lib.Utils.isNullOrUndefined(config.lib_dynamodb)) {
      throw new Error('[js-server-helper-verify-store-dynamodb] config.lib_dynamodb is required (pass Lib.DynamoDB)');
    }
