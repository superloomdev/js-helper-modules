// Info: Config validator for js-server-helper-auth-store-sqlite.
  ...
  *********************************************************************/
  validateConfig: function (Lib, config) {
    ...
    if (
      ...
      throw new Error('[js-server-helper-auth-store-sqlite] config must be an object');
    }
    ...
    if (
      ...
      throw new Error('[js-server-helper-auth-store-sqlite] config.table_name is required');
    }

    // lib_sql is required - the caller must inject the SQLite helper
    if (Lib.Utils.isNullOrUndefined(config.lib_sql)) {
      throw new Error('[js-server-helper-auth-store-sqlite] config.lib_sql is required (pass Lib.SQLite)');
    }
