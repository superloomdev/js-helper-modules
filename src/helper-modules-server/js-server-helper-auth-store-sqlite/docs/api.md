...
The adapter validates the config, builds its own `Lib` (Utils + Debug) and `ERRORS` catalog internally, and returns a ready-to-use Store interface. Each call returns an independent Store; multiple Auth instances (different `ACTOR_TYPE` values, different tables) coexist in the same process and share the same `Lib.SQLite` driver helper if it is the same instance.
