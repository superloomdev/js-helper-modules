# Schema - helper-cache-store-aws-elasticache

## Key Structure

ElastiCache is a flat keyspace. Every cache entry is stored under one string key composed from three parts:
...
This adapter JSON-serializes every value before handing it to ElastiCache. The cache module passes raw JavaScript objects; the adapter owns serialization. No additional encoding or transformation happens beyond JSON.stringify/parse.

## TTL

TTL is native to ElastiCache. `setCache` with a `ttl_seconds` value issues `SET key value EX ttl_seconds` under the hood (via `Lib.KV.set`). ElastiCache deletes the key automatically when the TTL expires - no application-side sweep is needed.

When `ttl_seconds` is omitted, the key has no expiry and persists until explicitly deleted.

## No Secondary Index

ElastiCache has no partition key or sort key concept. The only index is the keyspace itself. `deleteCacheByPrefix`, `clearCache`, and `listCacheCodes` use `SCAN MATCH prefix*`, which iterates every key in the database and filters after retrieval. See [Configuration](configuration.md#deletecachebyprefix-clearcache-and-listcachecodes-complexity) for the O(N) cost analysis.
...
ElastiCache deletes an expired key lazily, on access, and `SCAN` may still return a key whose TTL has passed but which has not yet been reclaimed. `listCacheCodes` can therefore report a `cache_code` that a following `getCache` reports as a miss.

This is accepted, not worked around. Filtering would cost one `GET` per scanned key, turning a single `SCAN` pass into N round trips. The window is bounded by ElastiCache's own reclamation and is small in practice. Callers that need list-then-get to agree must tolerate a miss on the get, which is the normal cache contract anyway.
