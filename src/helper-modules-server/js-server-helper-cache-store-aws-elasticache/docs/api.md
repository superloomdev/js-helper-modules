# API Reference - helper-cache-store-aws-elasticache

This adapter implements the 9-method store contract consumed by `helper-cache` (7 required + 2 lock methods). This document focuses on the ElastiCache-specific semantics.
...
This adapter owns serialization. `setCache` JSON-stringifies the value before handing it to `Lib.KV.set`; `getCache` JSON-parses the stored string before returning it to the cache module. The cache module passes raw JavaScript objects. A serialization failure returns `CACHE_ELASTICACHE_SERIALIZATION_FAILED`.

## Store Contract

### `getCache(instance, namespace, cache_code)`

Composes a flat ElastiCache key `KEY_PREFIX + namespace + KEY_SEPARATOR + cache_code` and delegates to `Lib.KV.get`. Returns `value: null` on a miss (key absent or expired via native ElastiCache TTL). The stored JSON string is deserialized before being returned.
...
Composes the key, JSON-serializes the value, and delegates to `Lib.KV.set`. `ttl_seconds` is positional and optional - when absent, the key has no expiry. ElastiCache handles expiry natively via `SET key value EX ttl_seconds`.
