// Info: AWS ElastiCache store adapter for helper-cache. Fully independent
// module that owns its own CONFIG, ERRORS, and Validators. Composes a
// flat ElastiCache key from namespace and cache_code using KEY_PREFIX and
// KEY_SEPARATOR, then delegates to Lib.KV (the kv-aws-elasticache driver).
//
...
// which is O(N) over the entire keyspace. ElastiCache runs the Redis or
// Valkey engine, which exposes a flat keyspace with no partition or sort
// key, so no prefix-scoped index exists. The cost implications on
// node-based versus serverless ElastiCache are documented in this
    ...
    *********************************************************************/
    getCache: async function (instance, namespace, cache_code) {
      ...
      try {
        ...
        Lib.Debug.debug('[helper-cache-store-aws-elasticache] getCache deserialization failed', {
          namespace: namespace,
    ...
    *********************************************************************/
    setCache: async function (instance, namespace, cache_code, value, ttl_seconds) {
      ...
      try {
        ...
        Lib.Debug.debug('[helper-cache-store-aws-elasticache] setCache serialization failed', {
          namespace: namespace,
          cache_code: cache_code,
          error: err && err.message
        });
  ...
  const _Store = {
    ...
    ******************************************************************/
    logDriverFailure: function (method, driver_error) {
      Lib.Debug.debug('[helper-cache-store-aws-elasticache] ' + method + ' failed', {
