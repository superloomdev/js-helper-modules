...
- `helper-instance` - injected as `Lib.Instance`
- `helper-http-gateway` - injected as `Lib.HttpGateway` (cookie descriptor building via `buildCookie`; serialization into `Set-Cookie` headers happens at the gateway boundary, not inside auth)
...
setupNewStore(instance) -> { success, error } | async:yes
  ...
  Shipped NoSQL adapters (mongodb, dynamodb) return { success: false, error: AUTH_NOT_IMPLEMENTED } - provision the table/collection out-of-band via IaC. A custom store that omits the method entirely trips the parent's call-time capability gate and throws TypeError.
...
| AUTH_NOT_IMPLEMENTED | setupNewStore on NoSQL backends | Operation has no meaning on this backend; provision out-of-band |
...
- **`instance` is always first argument.** Every function reads `instance.time` for timestamps
...
- `docs/schemas.md` - validated contracts (CONFIG, JWT, call options), eight-method store contract, throw-vs-return, response envelope
...
- Storage adapters: see the README's "Storage Adapters" section for the list + selection rule. Per-backend schema, indexes, TTL, IaC notes, and store config shape live in each adapter package's own README (`helper-auth-store-*`)
