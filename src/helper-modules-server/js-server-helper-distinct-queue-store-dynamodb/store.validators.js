  ...
  *********************************************************************/
  validateConfig: function (store_config) {
    ...
    if (
      ...
      throw new Error('[js-server-helper-distinct-queue-store-dynamodb] STORE_CONFIG must be an object');
    }
    ...
    if (
      ...
      throw new Error('[js-server-helper-distinct-queue-store-dynamodb] STORE_CONFIG.table_name is required');
    }
    ...
    if (Lib.Utils.isNullOrUndefined(store_config.lib_dynamodb)) {
      throw new Error('[js-server-helper-distinct-queue-store-dynamodb] STORE_CONFIG.lib_dynamodb is required (pass Lib.DynamoDB)');
    }
