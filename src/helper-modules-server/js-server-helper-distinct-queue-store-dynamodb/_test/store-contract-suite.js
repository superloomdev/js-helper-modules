// Info: Shared contract test suite for DynamoDB store adapter.
...
@param {Function} deps.cleanTable      - Wipes the test table
...
module.exports = function buildContractSuite (deps) {
  ...
  const cleanTable = deps.cleanTable;
  ...
  return async function runContractSuite () {
    ...
    // Idempotent setup (table creation) runs once
    before(async function () {
      await store.setupNewStore(buildInstance());
    });

    // Isolate every test with a clean table
