...
Postgres store adapter for helper-auth. Implements the 8-method session store contract backed by Postgres via helper-sql-postgres. Part of [Superloom](https://superloom.dev).
...
It receives Utils, Debug, and Postgres through the `Lib` container injected by the caller. For the full dependency breakdown, see [`docs/configuration.md`](docs/configuration.md).

## Testing Status

| Tier | Runtime | Status |
|---|---|---|
| Contract + Integration | PostgreSQL 17 in Docker | [![Test](https://github.com/superloomdev/js-helper-modules/actions/workflows/ci-publish-helper-modules.yml/badge.svg?branch=main)](https://github.com/superloomdev/js-helper-modules/actions/workflows/ci-publish-helper-modules.yml) |
