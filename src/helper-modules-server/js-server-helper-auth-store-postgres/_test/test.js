// Info: Three-tier test suite for helper-auth-store-postgres.
//
...
const buildStore = function (table) {
  ...
  return AuthStorePostgresFactory(Lib, {
...
describe('Tier 1: store loader validation', function () {
  ...
  it('throws when table_name is empty string', function () {
    ...
    assert.throws(
      function () { AuthStorePostgresFactory(Lib, { table_name: '' }); },
  ...
  it('throws when table_name is null', function () {
    ...
    assert.throws(
      function () { AuthStorePostgresFactory(Lib, { table_name: null }); },
      /table_name is required/
    );
  ...
  it('returns a store object when config is valid', function () {
    ...
    const store = AuthStorePostgresFactory(Lib, { table_name: 'valid_table' });
    assert.ok(store);
...
describe('Tier 1: _Store identifier quoting', function () {
  ...
  it('Q rejects a table_name containing a double-quote', function () {
    ...
    assert.throws(
      function () {
        AuthStorePostgresFactory(Lib, { table_name: 'bad"table' });
      },
...
describe('Tier 1: value coercions', { concurrency: false }, function () {
  ...
  it('round-trips client_is_browser = true as native BOOLEAN and reads back as true', async function () {
  ...
  it('round-trips client_is_browser = false as native BOOLEAN and reads back as false', async function () {
...
describe('Tier 1: getSession hash guard', { concurrency: false }, function () {
...
describe('Tier 1: updateSessionActivity identity guard', { concurrency: false }, function () {
...
describe('Tier 1: setSession upsert preserves immutable columns', { concurrency: false }, function () {
...
describe('Tier 1: cleanupExpiredSessions deleted_count', { concurrency: false }, function () {
...
describe('Tier 1: refresh_token_hash and refresh_family_id round-trip', { concurrency: false }, function () {
...
describe('Tier 1: updateSessionActivity edge cases', { concurrency: false }, function () {
...
describe('Tier 1: deleteSessions bulk path', { concurrency: false }, function () {
...
describe('Tier 1: getSession not-found path', { concurrency: false }, function () {
...
describe('Tier 1: cleanupExpiredSessions on empty table', { concurrency: false }, function () {
...
describe('Tier 1: large multi-actor list isolation', { concurrency: false }, function () {
...
const buildAuth = function (overrides) {
  ...
  const config = Object.assign({
    Store:     AuthStorePostgresFactory(Lib, { table_name: TEST_TABLE }),
    ACTOR_TYPE: 'user',
    TTL_SECONDS: 3600,
